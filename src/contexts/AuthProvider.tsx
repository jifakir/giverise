...
const AuthProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const handleOAuthLogin = async (provider: string) => {
        if (provider) {
            ...
            const win = window.open(authLink, "auth", 'width=600,height=600,scrollbars=yes');
            let count = 0;
    ...
    return (
        <AuthContext.Provider value={{ user, token, authStatus, updateUser, handleOAuthLogin, handleBasicLogin, logout }}>
            {children}
