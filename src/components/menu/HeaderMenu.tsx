  ...
  <div className="w-[232px] py-3 rounded-lg drop-shadow-md bg-white px-4">
    <ul>
      <li>
        <a
          href="#"
          className="text-sm font-normal text-primary flex py-3 hover:text-primary-purple"
        >
          About Giverise
        </a>
      </li>
      ...
      <li>
        <a
          href="#"
          className="text-sm font-normal text-primary flex py-3 hover:text-primary-purple border-b border-[#EDF2F7]"
        >
          Tax & My Donations
        </a>
      </li>
    ...
    <div className="w-[232px] py-3 rounded-lg drop-shadow-md bg-white px-4">
      ...
      <ul>
        <li>
          <Link href={"/profile"}>
            <a className="text-sm font-normal text-primary flex py-3 hover:text-primary-purple items-center">
              <span className="mr-3 text-secondary text-xl hover:text-primary-purple">
                <User />
              </span>
              Your Profile
            </a>
        ...
        <li>
          <button
            onClick={onLogout}
            className="text-sm font-normal text-primary flex items-center py-3 hover:text-primary-purple border-b border-[#EDF2F7]"
          >
            <span className="mr-3 text-secondary text-xl hover:text-primary-purple">
              <Off />
            </span>
...
const HeaderMenu = ({ hideActions = false }) => {
  const { user, isNewSocialUser, logout } = useAuth();
  ...
  return (
    <>
      <div
        className={`${
          hideActions ? "justify-center" : ""
        } bg-white px-8 h-[70px] flex items-center border-b border-primary border-opacity-[0.08] smMax:bg-[#F8F9FC] smMax:h-[116px] smMax:border-0 smMax:items-end smMax:pb-6 smMax:px-5 xs:px-4`}
      >
        <Link href={"/"}>
          <a>
            <img
        ...
        {!hideActions ? (
          <>
            <div className="ml-12 relative w-[340px] mdMax:hidden">
              <input
                placeholder="Search Giverise"
                className="text-sm text-body w-full rounded-lg h-[38px] px-4 border border-[#E2E4E8] box-border focus:outline-none"
              />
            ...
            <div className="flex items-center ml-auto gap-4">
              ...
              {user && !isNewSocialUser ? null : (
                <button
                  onClick={openAuthModal}
                  className="bg-primary-purple rounded-md flex items-center px-8 hover:text-white h-10 text-base font-medium text-white smMax:hidden"
                >
                  Login
                </button>
              )}
              {user && !isNewSocialUser ? (
                <>
                  <DropdownMenu menu={<ProfileMenu onLogout={logout} />}>
                    {user.socialLogin?.avatar ? (
                      ...
                      <Avatar
                        size={24}
                        className="bg-[#FAFBFC] object-cover cursor-pointer text-primary text-sm font-semibold"
                      >
